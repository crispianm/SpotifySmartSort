def evaluate_data(data):
    ...
    for x in range(1, len(data)):
        ...
        if songA["key"] == songB["key"]:
            ...
            songA["key"] == (songB["key"] - 5) % 12
            or songA["key"] == (songB["key"] + 5) % 12
        ):
            keyTransition += 1
        ...
        if (
            songA["loudness_end"] * 0.7
            <= songB["loudness_start"]
            <= songA["loudness_end"] * 1.3
        ):
        ...
        if (
            songA["song_timbre_end"] * 0.7
            <= songB["song_timbre_start"]
            <= songA["song_timbre_end"] * 1.3
        ):
            timbreTransition += 1
        elif (
            songA["song_timbre"] * 0.7
            <= songB["song_timbre"]
            <= songA["song_timbre"] * 1.3
        ):
            timbreTransition += 1
        ...
        if (
            songA["danceability"] * lowerLimit
            <= songB["danceability"]
            <= songA["danceability"] * upperLimit
        ):
            danceabilityTransition += 1
        if (
            songA["energy"] * lowerLimit
            <= songB["energy"]
            <= songA["energy"] * upperLimit
        ):
            energyTransition += 1
        if (
            songA["speechiness"] * lowerLimit
            <= songB["speechiness"]
            <= songA["speechiness"] * upperLimit
        ):
            speechinessTransition += 1
        if (
            songA["acousticness"] * lowerLimit
            <= songB["acousticness"]
            <= songA["acousticness"] * upperLimit
        ):
            acousticnessTransition += 1
        if (
            songA["instrumentalness"] * lowerLimit
            <= songB["instrumentalness"]
            <= songA["instrumentalness"] * upperLimit
        ):
            instrumentalnessTransition += 1
        if (
            songA["liveness"] * lowerLimit
            <= songB["liveness"]
            <= songA["liveness"] * upperLimit
        ):
            livenessTransition += 1
        if (
            songA["valence"] * lowerLimit
            <= songB["valence"]
            <= songA["valence"] * upperLimit
        ):
            valenceTransition += 1
